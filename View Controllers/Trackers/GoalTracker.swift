...
class GoalKeeperTrackerViewController: UIViewController {
    ...
    var trackerData = GoalkeeperTrackerData()
    var dbInt: Int = 0
    ...
    @IBAction func setRedCard(_ sender: UIButton) {
        trackerData.redCard = true
    }
    @IBAction func addSave(_ sender: UIButton) {
        trackerData.saves += 1
        sender.setTitle("Сейв (\(trackerData.saves))", for: .normal)
    }
    @IBAction func addConceded(_ sender: UIButton) {
        trackerData.conceded += 1
        sender.setTitle("Пропущенный гол (\(trackerData.conceded))", for: .normal)
    }
        ...
        player.updateChildValues(
          [
            "gameStats": [
              ...
              "saves": trackerData.saves,
              "concede": trackerData.conceded,
